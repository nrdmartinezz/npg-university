---
title: "Advanced New Patient Phone Call"
description: "An 8.5-hour receptionist phone course on sales fundamentals, customer service, value-building, and objection handling for the new patient call."
subtitle: "Transform your receptionist into the ultimate conversion machine."
enrollUrl: "https://npguniversity.samcart.com/products/receptionist-phone-training-course/"
instructor: "New Patient Group"
featured: true
image: "/images/home/course-phone.png"
imageAlt: "Front desk team member on a call in a modern orthodontic office"
tracks:
  - receptionist
positions:
  - track: receptionist
    order: 3
includes:
  - "8.5 hours of on-demand video you keep forever"
  - "Full access on any internet-connected device"
  - "More than 6 printable scripts and flowcharts"
  - "Online testing and progress tracking"
  - "3 private online coaching sessions for your practice and team"
  - "Those coaching sessions are recorded and provided to you"
---

Advanced phone training built to increase new patients, same-day starts, and revenue. Competition creates shoppers. This course teaches the team how to turn them into value-based buyers.

This 8-plus-hour receptionist phone training course takes the front desk through a transformation of the new patient phone call. In a competitive orthodontic and dental market, where treatment can look like a commodity, the practice wins through customer service and the ability to describe the practice in a remarkable way.

The goal is fewer no-shows and a new patient who arrives motivated and ready to say yes the same day. The team trains on sales fundamentals, customer-service language, edification and value-building, and objection handling.

## Curriculum

### Introduction

- Preparing for Success — Part 1
- Preparing for Success — Part 2
- Preparing for Success — Part 3
- What Exceptional Looks Like — Mystery Call Examples

### Printable scripts and flow charts

- Recipe for Same Day Starts — three strategies on every new patient phone call
- 6 Steps to the New Patient Phone Call
- 3 Steps to Overcoming Price Inquiries
- Responses to the most common and uncommon phone call scenarios
- Reducing no-shows and late cancellations

### Taking control

- Introduction
- Mistakes to avoid
- Taking control on the front end of the call
- Open-ended questions — Parts 1 and 2
- How to get an answer
- Review, role play, and implementation
- Exam — Taking Control

### 5-star customer service and hospitality

- Making an excellent first impression
- Consistency, placing patients on hold, and cross training
- Becoming the expert, positive connotation, politeness, and addressing patients
- Communication skills — Parts 1, 2, and 3
- Review, role play, and implementation
- Exam — 5 Star Customer Service

### Creating value

- Speaking the consumer language and building rapport
- The importance of selling you
- The 6 edification topics of practice success
- The value proposition exercise
- Exam — Creating Value

### 6 steps to the new patient phone call

- VIP greeting, identifying the caller, and taking control
- Dual option close, data capture, and confirmation and education
- Exam — 6 Steps to the New Patient Phone Call

### Overcoming price inquiries

- Positive connotation, value proposition, and clinical education
- Willingness to say goodbye
- Exam — Overcoming Pricing Inquiries

### Handling insurance inquiries

- Insurance positive connotation
- Practice edification and insurance edification
- Exam — Handling Insurance Inquiries

### Final exam

- Testing that covers the on-demand video

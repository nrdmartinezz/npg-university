---
title: "The Four Essential Steps to Presenting Money to Today’s Consumer"
description: "Brian Wright’s treatment coordinator course on presenting fees to a consumer who decides differently than they used to."
subtitle: "Skyrocket conversion, starts, production, revenue, cash flow, and efficiency."
enrollUrl: "https://npguniversity.samcart.com/products/money/"
instructor: "Brian Wright, Founder & CEO"
price: 375
tracks:
  - treatment-coordinator
positions:
  - track: treatment-coordinator
    order: 1
includes:
  - "PDF printable documents you keep"
  - "Team exercises and role plays"
  - "Expert guidance with Brian Wright"
  - "Keep the course forever"
---

Join Founder and CEO Brian Wright for this advanced course on presenting money. It is aimed at conversion, starts, production, revenue, cash flow, and efficiency.

The sales page cites Dental Economics on conversion in dentistry dipping to 32%, and a similar look at orthodontics dipping into the high 40s. The argument is that the consumer has changed, and the course teaches skill sets for that change.

It is written for the treatment coordinator and anyone else who presents financials, and the same skills are presented as useful for other roles. You keep access for life.

## Skill sets

- Advanced leadership
- Advanced hospitality
- Advanced sales strategies and techniques
- Advanced consumer psychology
- Advanced verbiage and presentation
- Advanced communication

## Why it matters on the sales page

The course is framed as a response to shoppers who compare coverage and fees. It covers how to present in a way that adds more value than the other practices they are calling.
